...
class ConstitutiveLaws():
    ...
    def sigma_shear(self):
        """ ---------- Out-of-plane shear stiffness (Reissner-Mindlin) -- vectorised ---------------
            Vectorised analogue of stress.sigma_shear in Stresses_mixreinf. Closed-form, linear:

                G    = Ec / (2 * (1 + nu))            # shear modulus
                txz  = 5/6 * G * gxz                  # 5/6 = standard shell shear correction
                tyz  = 5/6 * G * gyz

            Reads gxz, gyz from self.e[:, :, 3:4] and [:, :, 4:5]; requires self.e to carry the
            full 5-component strain vector. Returns txz, tyz each of shape (n_tot, nl, 1).
        -----------------------------------------------------------------------------------------"""
        gxz = self.e[:, :, 3:4]
        gyz = self.e[:, :, 4:5]
        G   = self.Ec / (2.0 * (1.0 + self.v))
        txz = (5.0 / 6.0) * G * gxz
        tyz = (5.0 / 6.0) * G * gyz
        return txz, tyz
    ...
    def out(self, sprev_klij = 0, do_cracked=True):
        """ ------------------------------------------- Define Output---------------------------------------------------
            --------------------------------------------    INPUT: -----------------------------------------------------
            - self.e:       [ex_klij,ey_klij,gxy_klij,gxz_klij,gyz_klij] strain state in integration point
                            (out-of-plane shear strains gxz, gyz needed for the 5-component output below)
        ...
        # 3 Output: numerical floor on txy.imag, matching stress.out() in Stresses_mixreinf:
        #     ff = Ec / 100
        #     if abs(txy.imag) < abs(gxy.imag) * ff/2:
        #         txy = cplx(txy.real, gxy.imag * ff/2)
        # Prevents the principal-direction CMM from yielding ~zero shear tangent when
        # (ex - ey) is large enough that the complex-step perturbation in gxy doesn't
        # propagate through the Mohr sqrt. Only the imag part is modified -- txy.real
        # (the physical stress feeding sh / fi) is left unchanged.
        ff_floor = self.Ec / 100.0
        cond     = np.abs(txy.imag) < np.abs(gxy.imag) * ff_floor / 2.0
        new_imag = np.where(cond, gxy.imag * ff_floor / 2.0, txy.imag)
        txy      = (txy.real + 1j * new_imag).astype(txy.dtype)

        # 4 Out-of-plane shear (analytical) -- requires self.e to be 5-component
        txz, tyz = self.sigma_shear()

        s = np.stack((sx, sy, txy, txz, tyz), axis = 2)
